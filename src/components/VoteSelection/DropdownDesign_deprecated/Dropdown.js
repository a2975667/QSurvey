/* sourced from https://github.com/fraserxu/react-dropdown/blob/master/index.js */
import { Component, createRef } from 'react'
import classNames from 'classnames'

const DEFAULT_PLACEHOLDER_STRING = '0'
...
class Dropdown extends Component {
  ...
  renderOption (option) {
    ...
    const dataAttributes = Object.keys(option.data || {}).reduce(
      ...
      // if else dropdown for vote and credit. use CSS to create four span inside the div
      // --> adjust the selected value in the enxt segment.
    return (
      <div
        key={value}
  ...
  render () {
    ...
    // then update the selected value here?
    const value = (<div className={placeholderClass}>
        <div className="ratingValue">{placeHolderValue>0 ? '+' + placeHolderValue : placeHolderValue}</div>
        <div className="rating">{placeHolderValue==0 ? 'rating' : "ratings"}</div>
        <div className="dollarSign">$</div>
        <div className="costValue">{placeHolderValue*placeHolderValue}</div>
    </div>)
